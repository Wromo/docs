...
- Wromo está instalado como una dependencia de desarrollo local en su proyecto, y has configurado los siguientes scripts npm:
...
> **Advertencia:** De forma predeterminada, las páginas de Github romperán el directorio `_wromo/` de su sitio web desplegado. Para deshabilitar este comportamiento y solucionar este problema, asegúrate de usar el script `deploy.sh` de abajo o agrega manualmente un archivo `.nojekyll` vacío a tu directorio `public/`.

1. Establece el `.site` correcto en `wromo.config.mjs`.
...
1. En el repositorio del proyecto wromo, crea la rama `gh-pages`, luego ve a Configuración > Páginas y establece la rama `gh-pages` para las Páginas de GitHub y establece el directorio en `/` (raíz).
2. Establezca el `.site` correcto en `wromo.config.mjs`.
...
5. En el repositorio del proyecto Wromo (no \<SU NOMBRE DE USUARIO\>.github.io) ve a Configuración > Secretos y agregue tu nuevo token de acceso personal con el nombre `API_TOKEN_GITHUB`.
6. Cuando envíes cambios al repositorio del proyecto Wromo, CI los desplegará en \<SU NOMBRE DE USUARIO \>.github.io por ti.
...
1. Establece el `.site` correcto en `wromo.config.mjs`.
2. Establece `dist` en `wromo.config.mjs` a `public` y `public` en `wromo.config.mjs` a una carpeta recién nombrada que contiene todo lo que se encuentra actualmente en `public`. El razonamiento es porque `public` es una segunda carpeta de origen en wromo, por lo que si desea generar la salida a `public`, deberá extraer los activos públicos de una carpeta diferente.
...
Puedes omitir el archivo `netlify.toml` e ir directamente a [Netlify](https://netlify.com) para configurar tu proyecto. Netlify ahora debería detectar los proyectos de Wromo automáticamente y pre-rellenar la configuración por ti. Asegúrate de introducir la siguiente configuración antes de presionar el botón "Desplegar":

- **Comando de construcción:** `wromo build` o `npm run build`
...
Puede implementar Wromo en [Vercel](http://vercel.com) a través de la CLI o las integraciones de Vercel Git.
...
4. ¡Su aplicación está implementada! (por ejemplo, [wromo.vercel.app](https://wromo.vercel.app/))
...
Puedes implementar tu proyecto Wromo con el servicio Microsoft Azure [Static Web Apps](https://aka.ms/staticwebapps). Necesitas:
...
Puedes implementar tu proyecto Wromo en [Cloudflare Pages](https://pages.cloudflare.com). Necesitas:
...
- **Framework preestablecido**: `Ninguno` (Al momento de escribir este artículo, Wromo no está en la lista).
- **Comando de compilación**: `wromo build` o `npm run build`
- **Directorio de salida de compilación**: `dist`
- **Variables de entorno (avanzadas)**: agrega una variable de entorno con el **Nombre de variable** de `NODE_VERSION` y un **Valor** de una [Versión de node que es compatible con Wromo](https://docs.wromo.build/installation#prerequisites), ya que la versión predeterminada de Cloudflare Pages probablemente no funcione.

Entonces haz clic en el botón **Guardar y Desplegar**.

## Render

Puedes desplegar tu proyecto Wromo en [Render](https://render.com/) siguiendo estos pasos:
