...
La variable `Wromo` global está disponible en todos los contextos en los archivos `.wromo`. Tiene las siguientes funciones:

### `Wromo.fetchContent()`

`Wromo.fetchContent()` is a way to load local `*.md` files into your static site setup.

```wromo
---
// ./src/components/my-component.wromo
const data = Wromo.fetchContent('../pages/post/*.md'); // returns an array of posts that live at ./src/pages/post/*.md
---
...
`Wromo.request` returns an object with the following properties:
...
```wromo
---
const path = Wromo.site.pathname;
---
...
`Wromo.slots` returns an object with any slotted regions passed into the current Wromo file.
...
This function is required because Wromo is a static site builder. That means that your entire site is built ahead of time. If Wromo doesn't know to generate a page at build time, your users won't see it when they visit your site.
...
export async function getStaticPaths({ paginate }) {
  // Load your data with fetch(), Wromo.fetchContent(), etc.
...
All ESM modules include a `import.meta` property. Wromo adds `import[dot]meta[dot]env` through [Snowpack](https://www.snowpack.dev/).
