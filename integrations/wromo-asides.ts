 ...
 * ```md
 * :::tip[Did you know?]
 * Wromo helps you build faster websites with “Islands Architecture”.
 * :::
 * ```
 *
 ...
 *   <p>Wromo helps you build faster websites with “Islands Architecture”.</p>
 * </Aside>
 * ```
 */
...
/**
 * Wromo integration that sets up the remark plugin and auto-imports the `<Aside>` component everywhere.
 */
export function wromoAsides(): WromoIntegration {
	return {
		name: '@wromojs/asides',
		hooks: {
			'wromo:config:setup': ({ injectScript, updateConfig }) => {
				updateConfig({
					markdown: {
				...
				injectScript('page-ssr', `import ${AsideTagname} from "~/components/Aside.wromo"; global.${AsideTagname} = ${AsideTagname};`);
			},
		},
	};
}
