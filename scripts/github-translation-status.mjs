 ...
 * the current Wromo documentation translation status.
 * 
...
class GitHubTranslationStatus {
	...
	async update () {
		...
		let intro = dedentMd`
			...
			[docs.wromo.build](https://docs.wromo.build/) into one of the languages listed below,
			...
			[i18n guide](https://github.com/Wromo/docs/tree/main/src/i18n/README.md) to learn about
	...
	renderCreatePageButton (lang, filename) {
		...
		const createUrl = new URL(`https://github.com/Wromo/docs/new/main/src/pages/${lang}`);
...
const githubTranslationStatus = new GitHubTranslationStatus({
	...
	githubRepo: process.env.GITHUB_REPOSITORY || 'Wromo/docs',
