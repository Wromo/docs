...
export class HtmlPage {
	 ...
	 * The full page URL.
	 * 
	 * Example: `https://docs.wromo.build/en/getting-started/`
	 */
	...
	constructor ({
		...
		this.anchors = htmlparser2.DomUtils
			.getElementsByTagName('a', this.dom, true);
		...
		const ids = this.findAll(el => Boolean(el.attribs.id))
			.map(el => el.attribs.id);
		this.hashes = [...anchorNames, ...ids]
			.map(name => `#${name}`);

		// Check if the page redirects somewhere else using meta refresh
		const metaRefreshElement = this.findFirst(el => (
		...
		const linkCanonicalElement = this.findFirst(el => (
			el.tagName.toLowerCase() === 'link' &&
			el.attribs['rel']?.toLowerCase() === 'canonical'
		));
		...
		const mainContentParentWithLang = this.mainContent &&
			this.findParent(this.mainContent, el => Boolean(el.attribs?.lang));
		...
		this.isLanguageFallback = (
			Boolean(this.pathnameLang) &&
			this.pathnameLang !== 'en' &&
			this.mainContentLang === 'en'
		);
	}
