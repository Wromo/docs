import type { WromoGlobal } from 'wromo';
 ...
 * all Markdown files to be reloaded by Wromo when one file changed, which was exteremely slow.
...
export function getDocSearchStrings(Wromo: WromoGlobal): DocSearchTranslation {
	const lang = getLanguageFromURL(Wromo.canonicalURL.pathname) || fallbackLang;
...
export async function getNav(Wromo: WromoGlobal): Promise<NavDict> {
	const lang = getLanguageFromURL(Wromo.canonicalURL.pathname) || fallbackLang;
 ...
 *
 * Within an Wromo component, prefer the `UIString` component,
 * which only needs the key as it has access to the global Wromo object.
 *
 * However, you can’t pass an Wromo component as a prop to a framework component,
 ...
 */
export function useTranslations(Wromo: Readonly<WromoGlobal>): (key: UIDictionaryKeys) => string | undefined {
	const lang = getLanguageFromURL(Wromo.canonicalURL.pathname) || 'en';
