...
title: デプロイ
description: WromoのWebサイトをデプロイする複数の方法。
---
...
- Wromoはプロジェクトのローカルのdev Dependencyにインストールされ、以下のnpmスクリプトが設定されています。
...
> これらのガイドでは、Wromoサイトを静的にデプロイする手順を説明します。Wromoは、SSR(Server Side Rendering)にも対応しています。
>
> 📚 [WromoプロジェクトでSSRを有効にする](/ja/guides/server-side-rendering/)についてもっと読む。
...
[GitHub Actions](https://github.com/features/actions)を使って、WromoサイトをGitHub Pagesにデプロイすると、自動的にサイトをビルドしてデプロイできます。これを行うには、ソースリポジトリがGitHubにホストされている必要があります。

1. `wromo.config.mjs`で[`site`](/ja/reference/configuration-reference/#site)と、必要なら[`base`](/ja/reference/configuration-reference/#base)オプションを設定します。
        ...
        steps:
          ...
          - name: Deploy to GitHub Pages
            ...
            with:
              ...
              # `./dist`はWromoがデフォルトでアウトプットするディレクトリです。
...
これであなたのサイトが公開されたはずです！ Wromoプロジェクトのリポジトリに変更をプッシュすると、GitHub Actionが自動的にデプロイしてくれます。

### Travis CI

1. `wromo.config.mjs`に正しい`.site`を設定します。
...
1. `wromo.config.mjs`に正しい`.site`を設定します。
2. `wromo.config.mjs`で現在の`dist`を`public`に設定し、現在の`public`を、現在 `public`にあるすべてのファイルを保持している新しい名前のフォルダーに設定します。その理由は、`public`はデフォルトでwromoのセカンドソースフォルダなので、`public`に出力したい場合は、別のフォルダからパブリックアセットを引っ張ってくる必要があるからです。 `wromo.config.mjs` は以下のような感じになります。
...
`netlify.toml`ファイルをスキップして、直接[Netlify](https://netlify.com/)にアクセスして、プロジェクトの設定を行えます。NetlifyはWromoプロジェクトを自動的に検出し、あなたに代わって設定をあらかじめ行ってくれるはずです。"Deploy"ボタンを押す前に、以下の設定が入力されていることを確認してください。

- **Build Command:** `wromo build`または`npm run build`
...
[Vercel](http://vercel.com/)へのWromoのデプロイは、CLIまたはVercelのgitインテグレーションを使用すれば、設定なしで可能です。

### CLI

1. [Vercel CLI](https://vercel.com/cli)をインストールし、`vercel`を実行してデプロイします。
2. Vercelは自動的にWromoを検出し、正しい設定を行います。
3. `Want to override the settings? [y/N]`と聞かれたら、"N"を選択します。
4. アプリケーションがデプロイされます！(例: [wromo.vercel.app](https://wromo.vercel.app/))。
...
3. Vercelは自動的にWromoを検出し、正しい設定を行います。
4. あなたのアプリケーションはデプロイされます！(例: [wromo.vercel.app](https://wromo.vercel.app/))
...
Microsoft Azure [Static Web Apps](https://aka.ms/staticwebapps)でWromoプロジェクトをデプロイするには以下が必要です。
...
Wromoプロジェクトを[Cloudflare Pages](https://pages.cloudflare.com/)にデプロイするには以下が必要です。
...
- **ビルド出力ディレクトリ:** `dist`
- **環境変数(アドバンスド)**: 現在、Cloudflare Pagesは、ビルド環境において、デフォルトで`NODE_VERSION = 12.18.0`をサポートしています。Wromoでは、`14.15.0`、`v16.0.0`以降が必要です。環境変数に **変数名** を`NODE_VERSION`とし、その **値** を[Wromoと互換性のあるNodeバージョン](/ja/install/auto/#前提条件)とするか、プロジェクトのNodeバージョンを`.nvmrc`または`.node-version`ファイルで指定して追加できます。
次に、**保存してデプロイする** ボタンをクリックします。

## Render

以下の手順で、Wromoプロジェクトを[Render](https://render.com/)上にデプロイできます。
...
[Buddy](https://buddy.works/)を使って、Wromoプロジェクトをデプロイできます。そのためには、以下のものが必要です。
