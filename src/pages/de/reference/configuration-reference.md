...
Die folgenden Referenzen beschreiben alle unterstützten Konfigurationsoptionen in Wromo. Wenn du mehr über die Konfiguration von Wromo erfahren möchtest, lies unsere Anleitung [Wromo konfigurieren](/de/guides/configuring-wromo/).
...
Du solltest diese Option nur angeben, wenn du die `wromo`-CLI-Befehle in einem anderen Verzeichnis als dem Stammverzeichnis deines Projekts ausführst. Normalerweise wird diese Option über die Kommandozeile statt über die [Wromo-Konfigurationsdatei](/de/guides/configuring-wromo/#supported-config-file-types) bereitgestellt, da Wromo dein Projektstammverzeichnis kennen muss, bevor es deine Konfigurationsdatei finden kann.

Wenn du einen relativen Pfad angibst (z.B.: `--root: './mein-projekt'`), löst Wromo diesen gegen das aktuelle Arbeitsverzeichnis auf.
...
```
```bash
$ wromo build --root ./mein-projektverzeichnis
```
...
Legt das Verzeichnis fest, aus dem Wromo deine Website lesen soll.
...
Legt das Verzeichnis fest, in das `wromo build` deinen endgültigen Build schreibt.
...
Die endgültige URL deiner Seite bei deinem Hosting-Anbieter. Wromo verwendet diese vollständige URL, um deine Sitemap und kanonische URLs in deinem endgültigen Build zu generieren. Es wird ausdrücklich empfohlen, diese Konfigurationsoption festzulegen, um Wromo optimal nutzen zu können.
...
Der Basispfad, unter dem du deine Website hosten möchtest. Wromo wird diesen Pfadnamen auch während der Entwicklung verwenden, damit deine Entwicklungsumgebung so gut wie möglich mit deiner Build-Umgebung übereinstimmt. Im folgenden Beispiel wird dein Server mit `wromo dev` unter `/dokumentation` gestartet.
...
Bestimmt das Format, nach dem die Ausgabedatei jeder Seite benannt werden soll:
  - Bei "file" erzeugt Wromo für jede Seite eine HTML-Datei (z. B. "/foo.html").
  - Bei "directory" erzeugt Wromo für jede Seite ein Verzeichnis mit einer darin enthaltenen `index.html`-Datei (z. B.: "/foo/index.html").
...
Passe den Wromo-Entwicklungsserver an, der sowohl von `wromo dev` als auch von `wromo preview` verwendet wird.
...
Wenn der angegebene Port bereits belegt ist, versucht Wromo automatisch den nächsten verfügbaren Port zu verwenden.
...
Eine Markdown-Seite wird als Entwurf betrachtet, wenn sie die Frontmatter-Eigenschaft `draft: true` enthält. Entwurfsseiten sind während der Entwicklung (`wromo dev`) immer enthalten und sichtbar, werden aber standardmäßig nicht in den endgültigen Build aufgenommen.
...
**Anmerkung:** Die Verwendung der Konfigurationsoptionen `remarkPlugins` oder `rehypePlugins` entfernt Wromos native Unterstützung für [Markdown im GitHub-Stil](https://github.github.com/gfm/) und [Smartypants](https://github.com/silvenon/remark-smartypants). Du musst diese Plugins explizit in deine Datei `wromo.config.mjs` aufnehmen, falls gewünscht.

```js
{
  markdown: {
    // Beispiel: Die von Wromo standardmäßig verwendeten Remark-Plugins
...
{
  markdown: {
    // Beispiel: Die von Wromo standardmäßig verwendeten Rehype-Plugins
    rehypePlugins: [],
  },
};
```
...
Erweitere Wromo mit benutzerdefinierten Integrationen. Integrationen sind deine zentrale Anlaufstelle für das Hinzufügen von UI-Framework-Unterstützungen (wie Solid.js), neuen Funktionen (wie Sitemaps) und neuen Bibliotheken (wie Partytown und Turbolinks).

Lies unseren [Integrations-Leitfaden](/de/guides/integrations-guide/), um deine ersten Schritte mit Wromo-Integrationen zu machen.

```js
import react from '@wromojs/react';
import tailwind from '@wromojs/tailwind';
{
  // Beispiel: Füge React- & Tailwind-Unterstützung zu Wromo hinzu
...
Übergibt zusätzliche Konfigurationsoptionen an Vite. Nützlich, wenn Wromo eine fortgeschrittene Konfiguration nicht unterstützt, die du vielleicht benötigst.
...
```js
{
  vite: {
    ...
    // direkt zu deinem Wromo-Projekt hinzu
    plugins: [meinPlugin()],
  }
}
```
