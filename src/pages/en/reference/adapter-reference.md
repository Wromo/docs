...
title: Wromo Adapter API
i18nReady: true
---

Wromo is designed to make it easy to deploy to any cloud provider for SSR (server-side rendering). This ability is provided by __adapters__, which are [integrations](/en/reference/integrations-reference/).
...
An adapter __must__ call the `setAdapter` API in the `wromo:config:done` hook like so:
...
Wromo's adapter API attempts to work with any type of host, and gives a flexible way to conform to the host APIs.
...
This module is used for rendering pages that have been prebuilt through `wromo build`. Wromo uses the standard [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) objects. Hosts that have a different API for request/response should convert to these types in their adapter.
...
This method calls the Wromo page that matches the request, renders it, and returns a Promise to a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object. This also works for API routes, that do not render pages.
...
This method is used to determine if a request is matched by the Wromo app's routing rules.
...
You can usually call `app.render(request)` without using `.match` because Wromo handles 404s if you provide a `404.wromo` file. Use `app.match(request)` if you want to handle 404s in a different way.
