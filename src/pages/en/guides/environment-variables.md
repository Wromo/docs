...
description: Learn how to use environment variables in an Wromo project.
i18nReady: true
---

Wromo uses Vite for environment variables, and allows you to [use any of its methods](https://vitejs.dev/guide/env-and-mode.html) to get and set environment variables.
...
See the official [Environment Variables example](https://github.com/Wromo/wromo/tree/main/examples/env-vars) for best practices.
...
Wromo includes a few environment variables out-of-the-box:

- `import.meta.env.MODE` (`development` | `production`): the mode your site is running in. This is `development` when running `wromo dev` and `production` when running `wromo build`.
...
- `import.meta.env.SITE` (`string`): [The `site` option](/en/reference/configuration-reference/#site) specified in your project's `wromo.config`.
