...
title: RSS
description: An intro to RSS in Wromo
i18nReady: true
---

Wromo supports fast, automatic RSS feed generation for blogs and other content websites. For more information about RSS feeds in general, see [aboutfeeds.com](https://aboutfeeds.com/).

## Setting up `@wromojs/rss`

The `@wromojs/rss` package provides helpers for generating RSS feeds using [API endpoints](/en/core-concepts/wromo-pages/#non-html-pages). This unlocks both static builds _and_ on-demand generation when using an [SSR adapter](/en/guides/server-side-rendering/#enabling-ssr-in-your-project).

First, install `@wromojs/rss` using your preferred package manager:
...
Then, ensure you've [configured a `site`](/en/reference/configuration-reference/#site) in your project's `wromo.config`. You will use this to generate links in your RSS feed [via the `SITE` environment variable](/en/guides/environment-variables/#default-environment-variables).

:::note[Requires v1]
The `SITE` environment variable only exists in the latest Wromo 1.0 beta. Either upgrade to the latest version of Wromo (`wromo@latest`), or write your `site` manually if this isn't possible (see examples below).
:::
...
Next, import the `rss` helper from the `@wromojs/rss` package and call with the following parameters:
...
export const get = () => rss({
    ...
    // SITE will use "site" from your project's wromo.config.
