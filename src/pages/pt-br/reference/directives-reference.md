...
**Diretivas de template** são um tipo especial de atributo HTML disponível dentro do template de qualquer componente Wromo (arquivos `.wromo`).

Diretivas de template são utilizadas para controlar o comportamento de um elemento ou componente de alguma forma. Uma diretiva de template pode habilitar alguma funcionalidade do compilador que facilitaria sua vida (como utilizar `class:list` ao invés de `class`). Ou então, uma diretiva pode dizer para o compilador do Wromo fazer algo especial com aquele componente (como hidratá-lo com `client:load`).

Esta página descreve todas as diretivas de template disponíveis para você no Wromo e como elas funcionam.
...
`class:list={...}` recebe um array de valores de classes e as converte em uma string de classes. Isso é inspirado pela famosa biblioteca utilitária [clsx](https://github.com/lukeed/clsx) feita por @lukeed, mas inserida diretamente no próprio Wromo.
...
**O valor não é automaticamente sanitizado pelo Wromo!** Tenha certeza de que o valor inserido é seguro, ou que você o sanitizou manualmente antes de passá-lo ao template. Esquecer disto vai te deixar exposto a [ataques de Cross Site Scripting](https://owasp.org/www-community/attacks/xss/).
...
`set:text={string}` injeta uma string de texto em um elemento, similar a se utilizar `el.innerText`. Diferente de `set:html`, o valor `string` que é passado é automaticamente sanitizado pelo Wromo.
...
**Você deve passar o framework correto do componente como valor!** Pois como o Wromo não executa o componente durante a build / no servidor, Wromo não sabe que framework seu componente utiliza a não ser que você o diga explicitamente.
...
Por padrão, Wromo automaticamente escopa as regras de CSS de `<style>` ao componente. Você pode remover esse comportamento com a diretiva `is:global`.

`is:global` faz com que o conteúdo de uma tag `<style>` seja aplicado globalmente na página quando o componente está incluso. Isso desabilita o sistema de escopo de CSS do Wromo. É o equivalente a envolver todos os seus seletores numa tag `<style>` com `:global()`.
...
Por padrão, Wromo irá processar, otimizar e fazer o bundle de tags `<script>` e `<style>` que ele ver na página. Você pode remover esse comportamento com a diretiva `is:inline`.

`is:inline` diz ao Wromo para deixar a tag `<script>` ou `<style>` como se fosse o HTML final resultante. Seus conteúdos não serão processados, otimizados ou passarão por bundle. Isso limita algumas funcionalidades do Wromo, como importar pacotes npm ou utilizar uma linguagem que compila para CSS como Sass. 
...
- Não terá suas referências de `import`/`@import`/`url()` resolvidas relativamente ao arquivo `.wromo`.
...
📚 Veja como [scripts no lado do cliente](/pt-br/core-concepts/wromo-components/#scripts-no-lado-do-cliente) funcionam em componentes Wromo.

### `define:vars`

`define:vars={...}` pode passar variáveis do front matter do seu componente no lado do servidor para o `<script>` ou `<style>` do cliente. Qualquer variável de front matter *serializável* é suportada, incluindo props passadas ao seu componente através de `Wromo.props`.

```wromo
---
const corPrimeiroPlano = "rgb(221 243 228)";
const corPlanoDeFundo = "rgb(24 121 78)";
const mensagem = "Wromo é incrível!";
---
...
`is:raw` instrui o compilador do Wromo a tratar qualquer elemento-filho do elemento como texto. Isso significa que toda a sintaxe de template especial do Wromo será ignorada dentro desse componente.
