...
description: Como migrar seu projeto para a versão mais recente do Wromo.
i18nReady: true
---

Este guia vai te ajudar a migrar de versões mais antigas do Wromo para a mais atualizada e recente versão.
...
Em 4 de Abril, 2022, nós lançamos a Beta do Wromo 1.0! 🎉 

**Nós não planejamos fazer mais mudanças radicais durante o período dessa beta até o lançamento oficial da v1.0.0 (planejado para o [fim de Julho, 2022](https://wromo.build/blog/wromo-1-release-update/)).**

Se qualquer mudança radical precisar se feita, nós iremos a apontá-la nesta seção.

## Migrando para a v1.0.0-beta.0

O lançamento `v1.0.0-beta.0` do Wromo não teve nenhuma mudança radical.
...
- `.sitemap` foi movida para a integração [@wromojs/sitemap](https://www.npmjs.com/package/@wromojs/sitemap).

Se você executar Wromo com uma configuração desatualizada, você irá ver um aviso com instruções em como atualizar. Veja nossa atualizada [Referência de Configuração](/pt-br/reference/configuration-reference/) para mais informações em como atualizar.

Leia [RFC0019](https://github.com/Wromo/rfcs/blob/main/proposals/0019-config-finalization.md) para um maior contexto sobre estas mudanças.

### Nova API de Markdown

Wromo v0.26 lançou uma completamente nova API de Markdown para o seu conteúdo. Isso inclui três principais mudanças para o usuário:
- Agora você pode usar `import`/`import()` para conteúdo markdown utilizando uma importação ESM diretamente.
- A nova API `Wromo.glob()`, para importações glob mais fáceis (especialmente para Markdown).
- **MUDANÇA RADICAL:** `Wromo.fetchContent()` foi removido e substituído por `Wromo.glob()`
- **MUDANÇA RADICAL:** Objetos Markdown tem uma nova interface atualizada.

```diff
// v0.25
- let todosPosts = Wromo.fetchContent('./posts/*.md');
// v0.26+
+ let todosPosts = await Wromo.glob('./posts/*.md');
```
...
Leia [RFC0017](https://github.com/Wromo/rfcs/blob/main/proposals/0017-markdown-content-redesign.md) para um maior contexto sobre estas mudanças.

### Novo Comportamento Padrão de Script

Tags `<script>` em componentes Wromo agora passam por build, bundle e são otimizadas por padrão. Isso completa uma mudança de longo prazo para fazer a nossa sintaxe de componentes Wromo mais consistente, correspondendo ao comportamento padrão otimizado que nossas tags `<style>` tem hoje.
...
- Nova diretiva `<script is:inline>`, para reverter uma tag `<script>` ao comportamento padrão anterior (sem passar por build, bundle, sem ser modificada pelo Wromo).
...
Veja como utilizar [scripts no lado do cliente](/pt-br/core-concepts/wromo-components/#scripts-no-lado-do-cliente) no Wromo para mais detalhes.

Leia [RFC0016](https://github.com/Wromo/rfcs/blob/main/proposals/0016-style-script-defaults.md) para mais contexto sobre estas mudanças.

### API `Wromo.request` Atualizada

`Wromo.request` foi mudada de nosso objeto customizado para um objeto `Request` padrão. Isto é parte de nosso projeto para usar mais APIs padrões da web, especialmente aonde SSR é uma preocupação. 

Isso inclui algumas mudanças para se estar atento sobre:

- Muda `Wromo.request` para se tornar um objeto [Request](https://developer.mozilla.org/pt-BR/docs/Web/API/Request).
- Move `Wromo.request.params` para `Wromo.params`.
- Move `Wromo.request.canonicalURL` para `Wromo.canonicalURL`.

Leia [RFC0018](https://github.com/Wromo/rfcs/blob/main/proposals/0018-wromo-request.md) para mais contexto sobre essas mudanças.


### Outras Mudanças

- Melhora a API de `Wromo.slots` para suportar a passagem de argumentos para slots baseados em funções. Isto permite componentes utilitários mais ergonômicos que aceitam uma função de callback como um filho.
- Atualiza a formatação da interface de linha de comando, especialmente de relatório de erros.
- Atualiza `@wromojs/compiler`, corrigindo alguns bugs relacionadas a utilização de RegExp no frontmatter.

## Migrando para a v0.25

### Integrações Wromo

A configuração `renderers` foi substituída por um novo e oficial, sistema de integrações! Isto permite algumas novas funcionalidades realmente animadoras para o Wromo. Você pode ler nosso guia [Usando Integrações](/pt-br/guides/integrations-guide/) para mais detalhes em como utilizar esse novo sistema.
...
Anteriormente, React, Preact, Svelte e Vue eram todos inclusos no Wromo por padrão. A partir da v0.25.0, Wromo não virá mais com qualquer renderizador integrado. Se você não tinha uma entrada `renderers` já definida no seu projeto, você agora precisará instalar esses frameworks por si mesmo.

Leia nosso [guia passo-a-passo](/pt-br/guides/integrations-guide/) para aprender como adicionar uma nova integração Wromo para o(s) framework(s) que você atualmente utiliza.
...
O novo sistema de integrações substitui o antigo sistema de `renderers`, incluindo os pacotes `@wromojs/renderer-*` publicados no npm. De agora em diante, `@wromojs/renderer-react` se torna `@wromojs/react`, `@wromojs/renderer-vue` se torna `@wromojs/vue`, e por aí vai.


**Para migrar:** atualize Wromo para a `v0.25.0` e então execute `wromo dev` ou `wromo build` com o seu antigo arquivo de configuração contendo a desatualizada configuração `"renderers"`. Você irá imediatamente ver um aviso te dizendo as exatas modificações que você precisa fazer em seu arquivo `wromo.config.mjs`, baseado na configuração atual. Você também pode atualizar seus pacotes por si mesmo, utilizando a tabela abaixo.

Para um passo-a-passo mais detalhado, veja nosso [guia passo-a-passo](/pt-br/guides/integrations-guide/) para aprender como substituir renderers existentes com uma nova integração de framework no Wromo.

```diff
# Instale suas novas integrações e frameworks:
# (Leia o passo-a-passo completo: https://docs.wromo.build/pt-br/guides/integrations-guide)
+ npm install @wromojs/lit lit
+ npm install @wromojs/react react react-dom
```
```diff
# Então, atualize seu arquivo `wromo.config.mjs`:
# (Leia o passo-a-passo completo: https://docs.wromo.build/pt-br/guides/integrations-guide)
...
Se você ver um aviso `"Cannot find package 'react'"` (ou similar) quando você inicia o Wromo, isso significa que você precisa instalar tal pacote em seu projeto. Veja nossa [nota em dependências de pares](/pt-br/guides/integrations-guide/#lidando-com-dependências-de-integrações) no guia de integrações para mais informações.
...
Como parte de nossa missão de manter o núcleo do Wromo o mais leve possível, nós movemos o componente integrado `Prism` de `wromo/components` para o pacote `@wromojs/prism`. Agora você pode importar este componente de `@wromojs/prism` assim:
...
Já que o pacote `@wromojs/prism` ainda vem junto com o núcleo do `wromo`, você não precisa instalar nada novo, nem adicionar Prism como uma integração! Porém, note que nós _planejamos_ extrair `@wromojs/prism` (e o syntax highlighting do Prism como um todo) para um pacote instalável e separado no futuro. veja a [referência da API do componente `<Prism />`](/pt-br/reference/api-reference/#prism-) para saber mais.
...
:::note
A nova estratégia de build é o padrão da v0.24. Se você tiver problemas, você pode continuar utilizando a antiga estratégia de build passando a flag `--legacy-build`. Por favor [abra uma issue](https://github.com/Wromo/wromo/issues/new/choose) para que possamos resolver problemas na nova estratégia de build.
:::

A v0.24 introduziu uma nova estratégia de *build estática* que modifica o comportamento de algumas funcionalidades. Em versões anteriores do Wromo isso estava disponível como um comportamento que você poderia optar por utilizando a flag: `--experimental-static-build`.
...
### Descontinuado: `Wromo.resolve()`

`Wromo.resolve()` permite que você consiga URLs resolvidas para assets que você gostaria de referenciar no navegador. Isto era mais comumente utilizado dentro das tags `<link>` e `<img>` para carregar arquivos CSS e imagens quando necessário. Infelizmente, isso não vai mais funcionar por conta do Wromo agora fazer build de assets em *tempo de build* ao invés de fazer em *tempo de renderização*. Você vai querer atualizar suas referências a assets para uma das opções seguras para o futuro a partir de agora:
...
Utilize uma importação ESM para adicionar algum CSS na página. Wromo detecta essas importações de CSS e então faz build, otimiza e adicionar o CSS para a página automaticamente. Esta é a forma mais fácil de migrar de `Wromo.resolve()` ao mesmo tempo que se mantém as etapas de build/bundle automáticas que o Wromo providencia.

```wromo
---
// Exemplo: Wromo irá incluir e otimizar este CSS para você automaticamente
import './estilo.css';
---
...
- Arquivos JavaScript
- Arquivos TypeScript
- Frontmatter de componentes Wromo
...
Você pode refenciar qualquer arquivo dentro do diretório `public/` com uma URL de caminho absoluto nos templates de seus componentes Wromo. Esta é uma boa opção por padrão para scripts externos, já que isto te permite controlar a tag `<script>` na página por si mesmo.

Note que esta abordagem pula o processamento, o bundle e otimizações de JavaScript que são providenciadas pelo Wromo quando você utiliza o método com `import` descrito acima. Porém, isso pode ser preferível para quaisquer scripts externos que já foram publicados e minificados separadamente do Wromo. Se o seu script foi baixado de uma fonte externa, então este método é provavelmente preferível.
...
Utilize uma importação ESM dentro de um elemento `<script hoist>` em seu template Wromo, e então Wromo irá incluir o arquivo JavaScript em sua build final. Wromo detecta estas importações de JavaScript no lado do cliente e então faz build, otimiza e adicionar o JavaScript à página automaticamente. Esta é a forma mais fácil de migrar de `Wromo.resolve()` enquanto se mantém as etapas de build/bundle automáticas que o Wromo providencia.
...
Note que o Wromo irá fazer bundle deste script externo com o resto do seu JavaScript no lado do cliente, e irá carregá-lo no contexto de script `type="module"`. Alguns arquivos JavaScript mais antigos podem não ter sido escritos para o contexto `module`, nesse caso, eles podem precisar ser atualizados para utilizar este método.
...
Isto funciona dentro de qualquer componente JavaScript ou Wromo, e retorna a URL resolvida para a imagem final. Assim que você tiver resolvido a URL, você pode utilizá-la em qualquer lugar dentro do template do componente.

```wromo
---
// Exemplo: Wromo irá incluir esta imagem na sua build final
import urlImg from './pinguin.png';
---
<img src={urlImg} />
```

Similar a forma com que Wromo lida com CSS, a importação ESM permite ao Wromo realizar algumas simples otimizações de build para você automaticamente. Por exemplo, qualquer asset dentro de `src/` que é importado utilizando uma importação ESM (ex: `import urlImg from '/pinguin.png'`) terá seu nome de arquivo em hash automaticamente. Isto te permite fazer um cache mais agressivo do arquivo no servidor, melhorando a performance do usuário. No futuro, Wromo pode adicionar mais otimizações como esta. 

**Dica:** Se você não gosta de importações ESM estáticas, Wromo também suporta importações ESM dinâmicas. Nós apenas recomendados esta opção se você prefere esta sintaxe: `<img src={(await import('./pinguin.png')).default} />`.
...
No Wromo v0.23+, conteúdo HTML não escapado em expressões não é mais possível.
...
A partir da v0.21, Wromo é construído com [Vite].
Por consequência, configurações escritas em `snowpack.config.mjs` devem ser movidas para `wromo.config.mjs`.
...
Em Wromo v0.21+, plugins Vite podem ser configurados em `wromo.config.mjs`.
...
Em Wromo v0.21+, plugins agora devem utilizar `viteConfig()`.
...
Em Wromo v0.21+, atalhos de importação podem ser adicionados em `tsconfig.json` ou `jsconfig.json`.
...
Em Wromo v0.21+, arquivos precisam ser referenciados com suas extensões, exatamente como estão no disco. Neste exemplo, `Div.tsx` precisaria ser referenciado como `Div.tsx`, não `Div.jsx`.
...
Anteriormente, você poderia criar mini componentes Wromo dentro do Frontmatter Wromo, utilizando sintaxe JSX ao invés da sintaxe de componentes Wromo. Isto sempre foi um tipo de hack, mas no novo compilador isto se tornou impossível de dar suporte. Nós esperamos reintroduzir esta funcionalidade em um lançamento futuro do Wromo utilizando uma API diferente e independente do JSX. 

Para migrar para a v0.21+, por favor converta todos os componentes Wromo JSX (qualquer componente Wromo criado dentro do frontmatter de outro componente) como seus próprios componentes isolados.
...
No Wromo v0.21+, um bug que foi introduzido faz com que seja obrigatório importações dentro de seus componentes estarem no topo do seu frontmatter.

```wromo
---
import Componente from '../components/Componente.wromo'
