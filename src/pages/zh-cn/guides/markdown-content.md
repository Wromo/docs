...
title: Markdown
description: 在 Wromo 中使用 Markdown
---

Markdown 内容通常用于创作长文本内容，例如博客文章和文档。Wromo 内置了 Markdown 支持并增加了一些功能，例如在 Markdown 中支持 JavaScript 表达式和 Wromo 组件。

## Markdown 页面

Wromo 将 `/src/pages` 目录中的任一 `.md` 文件视为一个页面。将文件放在此目录或其的任何一个子目录中，则将用文件的路径名自动构建页面路由。

📚 阅读更多关于 Wromo 的[基于文件的路由](/zh-cn/core-concepts/routing/)。

### 基本示例

在 Wromo 中使用 Markdown 的最简单方法就是在项目中创建一个 `src/pages/index.md` 主页路由。然后再将下面的基本模板复制到你的项目中，这样你就可以在项目主页路径中查看渲染后的 HTML。通常是 [http://localhost:3000/](http://localhost:3000/)。
...
Markdown 页面有一个用于指定 `layout` 的特殊 frontmatter 属性，它定义了 Wromo [布局组件](/zh-cn/core-concepts/layouts/)的相对路径。该组件将包装你的 Markdown 内容，提供页面骨架和任何其他包含的页面模板元素。
...
2. [`<slot />`](/zh-cn/core-concepts/wromo-components/#插槽) 将指定 Markdown 内容的默认显示位置。
...
`content` 属性还包含一个 `wromo` 属性，其中包含有关页面的其他元数据，例如完整的 Markdown `source` 和 `headers` 对象。
...
```json
{
  ...
  "title": "Wromo 0.18 Release",
  "date": "Tuesday, July 27 2021",
  "author": "Matthew Phillips",
  "description": "Wromo 0.18 is our biggest release since Wromo launch.",
  "draft": false,
  ...
  **/
  "wromo": {
    ...
    "source": "# Wromo 0.18 Release\nA little over a month ago, the first public beta [...]"
  },
  "url": ""
}
```

> 💡 `content` 属性中的 `wromo` 和 `url` 是唯一受到 Wromo 保护的属性。对象的其余部分则由你的 frontmatter 变量定义。

### 使用 Frontmatter 属性

任何 Wromo 组件（不仅仅是布局！）都可以将 Markdown frontmatter 中定义的值作为属性。你可以使用 YAML frontmatter 指定各种类型的数据，并从每篇博客文章中获取整个 Wromo 网站中使用的更丰富的元信息。

如上所述，就像在布局中一样，在任何 `.wromo` 文件中都可以使用它们。

### 标题 ID

Wromo 将使用 [github-slugger](https://github.com/Flet/github-slugger) 为 Markdown 文件中的所有标题自动生成 id。但是如果指定了自定义 id 不会被覆盖。
...
> ⚠️ 虽然设置了 `draft: true`，你的站点就不会生成该页面路由，但 `Wromo.glob()` 仍会返回**你所有的 Markdown 文件**。

不想在帖子存档或最新帖子列表中显示，则要在数据（例如标题、链接、描述）中排除草稿，所以请确保你的 `Wromo.glob()` 函数也**过滤了全部草稿**。

⚙️ 如果想要在构建时生成草稿页面：

在 `wromo.config.mjs` 中的 `markdown` 添加 `drafts: true`
...
💡 你还可以给 `wromo build` 传递 `--drafts` 标志来构建草稿页面！

## 增强 Markdown

除了支持标准 Markdown 语法外，Wromo 对其进行了扩展以使你的内容更具表现力。以下是一些仅限于 Wromo 中的 Markdown 功能。
...
你可以将 Markdown 文件直接导入到你的 Wromo 文件中！你可以用 `import` 导入单个指定页面，也可以使用 `Wromo.glob()` 导入多个指定页面。
...
// Also, you can import multiple files with Wromo.glob
...
你可以在你的组件脚本中导入[内置 Wromo Markdown 组件](/zh-cn/reference/api-reference/#markdown-)，然后在 `<Markdown></Markdown>` 标签之间写下你想要的 Markdown 内容。
...
<Layout>
  <Markdown>
    ...
    **Everything** supported in a `.md` file is also supported here!
    ...
    - Rich component support like any `.wromo` file!
...
> 注意：`<Markdown />` 组件在 SSR 中没有，且可能会在 v1.0 中被删除。尽可能避免使用它。要在模板中使用 Markdown，请使用单独的 `.md` 文件，然后用 [`import` Markdown](/zh-cn/guides/markdown-content/#导入-markdown) 将其作为组件添加到模板中。阅读此 [RFC 讨论](https://github.com/Wromo/rfcs/discussions/179)以了解更多信息。
...
你可以通过修改 `wromo.config.mjs` 来自定义 Markdown 解析。[你在这里可以阅读完整的相关内容](/zh-cn/reference/configuration-reference/#markdown-options)。

### Markdown 插件

Wromo 支持 第三方 [remark](https://github.com/remarkjs/remark) 和 [rehype](https://github.com/rehypejs/rehype) 插件以增强 Markdown。你可以在 `wromo.config.mjs` 中添加插件。

> **注：**Wromo 默认自带 [GitHub-flavored Markdown](https://github.com/remarkjs/remark-gfm) 支持并预启用 [remark-smartypants](https://github.com/silvenon/remark-smartypants)。如果启用自定义 `remarkPlugins` 或 `rehypePlugins` 将移除这些内置插件，如果还要用就需要显式添加这些插件。

#### 如何在 Wromo 中添加 Markdown 插件
...
Wromo 内置了对 [Shiki](https://shiki.matsu.io/) 和 [Prism](https://prismjs.com/) 的支持。这为以下内容提供了即时语法高亮支持：
...
export default {
  markdown: {
    shikiConfig: {
      ...
      // Note: Shiki has countless langs built-in, including .wromo!
...
1. 在你的 `@wromojs/markdown-remark` 配置中[设置 `syntaxHighlight: 'prism'`](#选择语法高亮器)。
...
4. 通过 `<link>` 标签将其添加到[你的页面的 `<head>` 中](/zh-cn/core-concepts/wromo-pages/#页面-html)。
