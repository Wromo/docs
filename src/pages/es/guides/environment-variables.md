...
description: Aprende a usar variables de entorno en un proyecto de Wromo.
i18nReady: true
---

Wromo usa Vite para las variables de entorno y le permite [usar cualquiera de sus métodos](https://vitejs.dev/guide/env-and-mode.html) para obtener y establecer variables de entorno.
...
Consulte el [ejemplo de variables de entorno](https://github.com/Wromo/wromo/tree/main/examples/env-vars) para aprender las prácticas recomendadas.
...
Wromo incluye algunas variables de entorno predeterminadas:

- `import.meta.env.MODE` (`development` | `production`): el modo en el que se ejecuta su proyecto. Esto es `development` al ejecutar `wromo dev` y `production` al ejecutar `wromo build`.
...
- `import.meta.env.SITE` (`string`): [la opción de configuración `site`](/es/reference/configuration-reference/#site) especificada en el `wromo.config` de su proyecto.
